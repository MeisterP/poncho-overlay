...
SRC_URI="amd64? ( andyetitmoves-${PV}-1_x86_64.tar.gz )"
...
src_install() {
	...
	find . -maxdepth 1 -mindepth 1 -type d ! -iname lib | xargs doins -r || die "doins data dirs failed"
	# Install needed config files:
	doins *.cs *.frag || die "doins config failed"
